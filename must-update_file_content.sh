...
ANALYSIS=$(/opt/homebrew/bin/openclaw agent --session-id must-update-file -m "read and analyze the Google Spreadsheet at https://docs.google.com/spreadsheets/d/1VsRoh9Wg3TSAGJNtJHnovIqxzqKT-1ti1xbM7_DPEG0 using win=completed, tie=in progress, lost=incomplete. Summarize what tasks are completed, in progress, and incomplete. Highlight anything urgent or needing attention." 2>&1 | grep -v "Gateway agent failed" | grep -v "falling back" | grep -v "gateway closed" | grep -v "loopback" | grep -v "compaction")
